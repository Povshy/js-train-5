...
function printIndexes(array) {
  ...
  for (const arrrayIndex in array) {
...
function modifyArray(array) {
  ...
  delete array[2];
  return console.log(String(array));
}
...
function printUppercase(array) {
  ...
  for (const big in array) {
    // Перетворюємо кожен елемент в верхній регістр та виводимо на консоль
    console.log(array[big].toUpperCase());
  }
}
...
function findMinMax([firstElement, ...restElements]) {
  ...
  let maxi = firstElement;
  for (let m = 0; m < restElements.length; m++) {
    if (restElements[m] < mini) {
      mini = restElements[m];
    }

    if (restElements[m] > maxi) {
      maxi = restElements[m];
    }
  }
...
function replaceNumbers(arr) {
  ...
  for (let n in arr) {
    if (arr[n] <= 10) {
      arr1[n] = "Less than or equal to 10";
    } else {
      arr1[n] = "Greater than 10";
    }
  }
  return arr1;
}
